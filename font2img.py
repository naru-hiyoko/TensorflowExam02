...
def font2imgSingle(src, charset, char_size, canvas_size, x_offset, y_offset, sample_count, sample_dir, label=0, filter_by_hash=True):
    font = ImageFont.truetype(src, size=char_size)
...
#        print("filter hashes -> %s" % (",".join([str(h) for h in filter_hashes])))
    ...
    for c in charset:
        ...
        if e:
            #e.save(os.path.join(sample_dir, "%d_%04d.jpg" % (label, count)))
            e.save(os.path.join(sample_dir, "{}_{}_{}.jpg".format(label, name, count)))
            count += 1
            if count % 100 == 0:
                print("processed %d chars" % count)
...
parser.add_argument('--src', dest='src', required=True, help='path of the source font')
parser.add_argument('--dst', dest='dst', required=False, help='path of the target font')
...
parser.add_argument('--single', action='store_true', help='output is single char')
parser.add_argument('--batch_mode', action='store_true', help='use fonts under asset')

args = parser.parse_args()

def defaultProcessing(args):
    ...
    if args.single:
        font2imgSingle(args.src, charset, args.char_size, args.canvas_size, args.x_offset, args.y_offset, args.sample_count, args.sample_dir, args.label)
    else:
        font2img(args.src, args.dst, charset, args.char_size,
                 args.canvas_size, args.x_offset, args.y_offset,
                 args.sample_count, args.sample_dir, args.label, args.filter)

def batchProcessing(args, src, label):
    if args.charset in ['CN', 'JP', 'KR', 'CN_T']:
        charset = locals().get("%s_CHARSET" % args.charset)
    if args.charset in ['US']:
        #charset = [c for c in open(args.charset).readline()[:-1].decode("utf-8")]
    ...
    if args.single:
        font2imgSingle(src, charset, args.char_size, args.canvas_size, args.x_offset, args.y_offset, args.sample_count, args.sample_dir, label)
    else:
        font2img(src, args.dst, charset, args.char_size,
                 args.canvas_size, args.x_offset, args.y_offset,
...
if __name__ == '__main__':
    if args.batch_mode:
        print('use batch mode')
        i = -1
        _asset_name = './assets'
        for font in os.listdir(_asset_name):
            i += 1
            if '.ttf' in font:
                print('{} {}'.format(i, font))
                batchProcessing(args, _asset_name + '/' + font, i)
        pass
    else:
        print('use default')
        defaultProcessing(args)
